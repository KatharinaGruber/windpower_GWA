...
if results_path + '/windpower_' + state + '_MERRA2_GWA.nc' not in glob.glob(outfile):
    ...
    wind = xr.open_mfdataset(mer_path + "/eff_ws/merra2_wind_USA_*.nc", chunks = {'time': 100})
    ...
    for it in range(numit):
        ...
        if outfile_temp not in glob.glob(results_path + "/wp_"+state+"_MERRA2_GWA_temp*.nc"):
            wps = windpower_simulation_merra2_large(wind.wh50,
                                                    ...
                                                    pd.to_datetime(turbine_data_mer_gwa.commissioning[ind].values[i1:i2]).year.values,
                                                    GWA)
            # adapt numbers of locations in dataset
            wps = wps.assign_coords(location = np.arange(i1,i2))
            # save temporary file
            wps.to_dataset(name='wp').to_netcdf(outfile_temp)
            wps.close()
            del(wps)
        i1 = i2
