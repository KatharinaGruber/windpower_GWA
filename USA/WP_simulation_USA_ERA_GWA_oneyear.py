...
wind = xr.open_mfdataset(era_path + "/eff_ws/era5_wind_USA_*.nc", chunks = {'time': 38}).sel(time=np.arange(year+'-01-01',str(int(year)+1)+'-01-01',dtype='datetime64[h]'))
alpha = xr.open_mfdataset(era_path + "/eff_ws/era5_alpha_USA_*.nc", chunks = {'time': 38}).sel(time=np.arange(year+'-01-01',str(int(year)+1)+'-01-01',dtype='datetime64[h]'))

# with GWA
outfile = results_path + '/oneyear/windpower_??_ERA5_GWA_'+year+'.nc'
turbine_data_era_gwa = pd.read_csv(usa_path + '/turbine_data_era_gwa.csv', parse_dates=['commissioning'])
if results_path + '/oneyear/windpower_' + state + '_ERA5_GWA_'+year+'.nc' not in glob.glob(outfile):
	...
	wps = windpower_simulation_era5(wind.wh100,
									...
									GWA)
	# save as netcdf
	wps.to_dataset(name='wp').to_netcdf(results_path+"/oneyear/windpower_"+state+"_ERA5_GWA_"+year+".nc")
